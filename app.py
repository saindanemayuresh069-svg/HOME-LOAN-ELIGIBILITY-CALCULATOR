...
with col2:
    st.markdown("""
    <h1 style='margin-bottom:0;'>🏠 Home Loan Eligibility Calculator</h1>
    <p style='margin-top:0; font-size:18px;'>
    Developed by <b><i>Mayuresh Saindane</i></b>
    </p>
    """, unsafe_allow_html=True)
...
with left:
    ...
    if incentive_type == "monthly":
        st.write("Enter last 6 months incentive")
        for i in range(6):
            val = st.number_input(f"Month {i+1}", value=0)
            monthly_incentives.append(val)

    elif incentive_type == "yearly":
        y1 = st.number_input("Year 1 Incentive", value=0)
        y2 = st.number_input("Year 2 Incentive", value=0)
        y3 = st.number_input("Year 3 Incentive", value=0)
